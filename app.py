...
# ==============================================================================
# [PART 2] Cash Equity Load (Full Grid + Attribution)
# ==============================================================================
@st.cache_data
def load_cash_equity_data(file):
    ...
    try:
        ...
        # -----------------------------------------------------------
        # [ATTRIBUTION DATA PREP]
        # -----------------------------------------------------------
        # Total Portfolio Prev MV
        ...
        # Contribution = Daily PnL / Total Portfolio Prev MV
        ...
        # Country_Ret = Country Daily PnL / Country Prev MV
        country_agg = merged.groupby(['기준일자', 'Country']).agg({
            'Daily_PnL_KRW': 'sum', 'Prev_MV_KRW': 'sum'
        }).reset_index()
        country_agg['Country_Ret'] = np.where(country_agg['Prev_MV_KRW']>0, 
                                              country_agg['Daily_PnL_KRW']/country_agg['Prev_MV_KRW'], 0)
        ...
        # Local Hedge Prep (Download USD/KRW)
        ...
        return df_perf, df_last, {'Sector':contrib_sector, 'Country':contrib_country}, country_agg, debug_logs, None
...
if menu == "Total Portfolio (Team PNL)":
    ...
    if uploaded_file_ce:
        ...
        if err: st.error(err)
            ...
            bm_prices = download_benchmarks_all(start_dt, end_dt)
            bm_ret = bm_prices.pct_change().fillna(0) if not bm_prices.empty else pd.DataFrame()
            bm_cum = (1 + bm_ret).cumprod() - 1 if not bm_ret.empty else pd.DataFrame()
            ...
            # Add Benchmarks
            if not bm_cum.empty:
                colors = {'US': 'grey', 'KR': 'silver', 'HK': 'lightgrey', 'JP': 'gainsboro'}
                for col in ['US', 'KR', 'HK', 'JP']:
                    if col in bm_cum.columns:
                        fig.add_trace(go.Scatter(x=bm_cum.index, y=bm_cum[col], name=col+' BM', line=dict(color=colors.get(col,'grey'), dash='dash', width=1)))
            st.plotly_chart(fig, use_container_width=True)
            ...
            with t1:
                ...
                if not country_daily.empty and not bm_ret.empty:
                    results = []
                    for ctry in country_daily['Country'].unique():
                        port_c_ret = country_daily[country_daily['Country']==ctry].set_index('기준일자')['Country_Ret']
                        bm_key = None
                        if ctry in ['US', 'USA']: bm_key = 'US'
                        elif ctry in ['KR', 'Korea']: bm_key = 'KR'
                        elif ctry in ['HK', 'Hong Kong']: bm_key = 'HK'
                        elif ctry in ['JP', 'Japan']: bm_key = 'JP'
                        
                        if bm_key and bm_key in bm_ret.columns:
                            alpha, beta, r2 = calculate_alpha_beta(port_c_ret, bm_ret[bm_key])
                            results.append({
                                'Country': ctry, 'Benchmark': bm_key,
                                'Alpha': f"{alpha:.2%}", 'Beta': f"{beta:.2f}", 'R2': f"{r2:.2f}"
                            })
                    if results: st.table(pd.DataFrame(results).set_index('Country'))
                    else: st.write("No sufficient data for regression.")
                else: st.write("No country data.")
            ...
            with t2:
                ...
                if df_contrib:
                    ...
                    # Need to get current holdings from df_last (only if last date matches)
                    # This is approximate if df_last is not strictly current.
                    # But load function ensures df_last is last record.
                    # Better: Filter Eq by max date inside load? (Already returned aggregate)
                    # Let's rely on df_last assuming it represents current state roughly.
                    # Correct way: use df_last where '기준일자' == max_date and Qty > 0
