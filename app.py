...
@st.cache_data
def download_benchmark_multi(start_date, end_date):
    """Download benchmarks for US, KR, HK, JP"""
    tickers = {
        'US': '^GSPC', 'KR': '^KS11', 'HK': '^HSI', 'JP': '^N225'
    }
    try:
...
def download_cross_assets(start_date, end_date):
    assets = {
        'S&P 500': '^GSPC', 'KOSPI': '^KS11', 'USD/KRW': 'KRW=X'
    }
    try:
...
# ==============================================================================
# [PART 2] Cash Equity Load (Full Grid + Weighted Local Return)
# ==============================================================================
@st.cache_data
def load_cash_equity_data(file):
    ...
    try:
        ...
        # 3. Country Mapping (Using Currency)
        if '통화' in eq.columns:
        ...
        cols_keep = ['원화평가금액', '원화총평가손익', '원화총매매손익', '외화평가손익', '외화총매매손익', '통화', '섹터', 'Country', '종목명', 'Market Value', 'Market Price']
        ...
        # (B) Local PnL & Return
        # Derive Local Cum PnL
        if '외화평가손익' in merged.columns:
            merged['Cum_PnL_Local'] = merged['외화평가손익'] + merged['외화총매매손익']
            # KRW Fallback
            if '통화' in merged.columns:
                is_krw = merged['통화'] == 'KRW'
                merged.loc[is_krw, 'Cum_PnL_Local'] = merged.loc[is_krw, 'Cum_PnL_KRW']
                # Also fix MV for KRW stocks
                if 'Market Value' in merged.columns:
                     merged.loc[is_krw, 'Market Value'] = merged.loc[is_krw, '원화평가금액']
        else:
            merged['Cum_PnL_Local'] = merged['Cum_PnL_KRW']

        merged['Daily_PnL_Local'] = merged.groupby('Ticker_ID')['Cum_PnL_Local'].diff().fillna(0)
        
        # Local Exposure (Prev MV Local)
        if 'Market Value' in merged.columns:
            merged['Prev_MV_Local'] = merged.groupby('Ticker_ID')['Market Value'].shift(1).fillna(0)
        else: merged['Prev_MV_Local'] = 0
        
        # Stock Local Return (Safe Division)
        merged['Stock_Ret_Local'] = np.where(merged['Prev_MV_Local'] > 0, 
                                             merged['Daily_PnL_Local'] / merged['Prev_MV_Local'], 0)
        ...
        # Portfolio Local Return = Weighted Average of Stock Local Returns
        # Weight = Stock Prev MV KRW / Portfolio Total Prev MV KRW
        ...
        daily_local_ret = merged.groupby('기준일자').apply(lambda x: (x['Stock_Ret_Local'] * x['Weight']).sum()).rename('Ret_Equity_Local')
        
        # Attribution Data (Contribution)
        # Contrib = Stock Daily PnL KRW / Portfolio Prev MV KRW
        ...
        # Return = Country PnL / Country Prev MV
        ...
        # Local Hedge Contribution
        # Contribution = Hedge PnL (in KRW) / Portfolio Prev MV (in KRW)
        # This ratio is currency-neutral (assuming Hedge PnL and Portfolio Value scale similarly with FX)
        df_perf['Ret_Hedge_Contrib'] = df_perf['Hedge_PnL_KRW'] / denom
        
        # Total Local (Hedged) = Local Equity + Hedge Contrib
        ...
        # Cumulative
        cols = ['Ret_Equity_KRW', 'Ret_Total_KRW', 'Ret_Equity_Local', 'Ret_Total_Local']
        for c in cols:
...
if menu == "Total Portfolio (Team PNL)":
    ...
    if uploaded_file_ce:
        ...
        if err: st.error(err)
            ...
            bm_returns = download_benchmark_multi(start_dt, end_dt)
            ...
            with t1:
                ...
                if not country_daily.empty and not bm_returns.empty:
                    results = []
                    bm_daily = bm_returns # Already daily
                    ...
                    else: st.write("Not enough data for regression.")
            ...
            with t2:
                ...
                if df_contrib:
                    ...
                    # Need to filter df_last by current balance logic?
                    # df_last is just last record per ticker. Some might have 0 qty.
                    # We need true current holdings.
                    # Let's grab rows from eq (merged not available here, but df_last has data)
                    # Actually df_last has '잔고수량'.
